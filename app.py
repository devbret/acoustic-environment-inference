...
def extract_audio_with_ffmpeg(src, dst, sr=TARGET_SR):
    cmd = ["ffmpeg", "-y", "-i", str(src), "-vn", "-ac", "1", "-ar", str(sr),
           "-hide_banner", "-loglevel", "error", str(dst)]
...
def energy_vad(y, sr, thresh_db=None, frame_ms=FRAME_MS, hop_ms=HOP_MS):
    ...
    idx = np.pad(
        idx,
        (0, max(0, len(y) - len(idx))),
        constant_values=mask[-1] if len(mask) > 0 else False,
    )[:len(y)]
...
def heuristic_scene(bands, rt60):
    rt = rt60 if rt60 is not None else 0.0
...
def analyze_file(path):
    ...
    try:
        ...
        parts = []
        if rt60_500:
            parts.append((rt60_500, c500))
        if rt60_1k:
            parts.append((rt60_1k, c1k))
        if parts:
        ...
        if result["hum"]["present"]:
            ...
            if fund:
                region = (
                    "50Hz_region" if 49.2 <= fund <= 50.8
                    else ("60Hz_region" if 59.2 <= fund <= 60.8 else "unknown_grid")
                )
...
def match_score(item_a: Dict[str, Any], item_b: Dict[str, Any]) -> Dict[str, Any]:
    ...
    score = (
        MATCH_WEIGHTS["fingerprint"] * fp_sim
